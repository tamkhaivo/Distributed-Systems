...
If you used Docker Compose for the server, you can dynamically spin up multiple client containers linking directly to it. To run 10 simultaneous clients and ensure any new code changes are built:
```bash
docker compose up --build --scale client=10 client
```
This will automatically build the latest client image and launch the specified number of isolated client containers, generating traffic to the server.
...
To close the server and client containers, run:
```bash
docker compose down
```
