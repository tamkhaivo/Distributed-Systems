...
3. **Physical (Hardware) Redundancy**
   ...
   * **Active Replication (State Machine Replication)**: Multiple identical servers process the exact same client requests concurrently. They coordinate via complex consensus protocols (like Paxos, Raft, or Zab) to agree on the correct sequence and output. If a node fails, the user experiences zero downtime, but the architecture requires immense compute overhead. *See [Module 10: Consensus Comparison](Module_10.md#544-raft-vs-paxos-a-comparative-deep-dive) for a detailed deep dive.*
